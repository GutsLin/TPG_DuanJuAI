...
app.post('/huobao-preset', async (c) => {
  ...
  for (const preset of HUOBAO_PRESET_SERVICES) {
    const rows = await db.select().from(schema.aiServiceConfigs).where(eq(schema.aiServiceConfigs.serviceType, preset.serviceType))
    const existing = rows.find(row => row.provider === preset.provider)
