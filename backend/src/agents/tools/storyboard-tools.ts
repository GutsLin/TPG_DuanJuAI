...
export function createStoryboardTools(episodeId: number, dramaId: number) {
  const readStoryboardContext = createTool({
    ...
    execute: async () => {
      ...
      const storyboardLinks = await db.select().from(schema.storyboardCharacters)
      const characterIdsByStoryboard = new Map<number, number[]>()
      for (const link of storyboardLinks) {
        const ids = characterIdsByStoryboard.get(link.storyboardId) || []
        ids.push(link.characterId)
        characterIdsByStoryboard.set(link.storyboardId, ids)
      }
      ...
      const payload = {
        ...
        existing_storyboards: existingStoryboards
          ...
          .map(sb => ({
            ...
            title: sb.title || '',
            scene_id: sb.sceneId,
            character_ids: characterIdsByStoryboard.get(sb.id) || [],
  ...
  const saveStoryboards = createTool({
    ...
    execute: async ({ storyboards }) => {
      ...
      const existingStoryboards = await db.select().from(schema.storyboards)
        .where(eq(schema.storyboards.episodeId, episodeId))
      const existingStoryboardIds = existingStoryboards.map(sb => sb.id)
  ...
  const updateStoryboard = createTool({
    ...
    execute: async ({ storyboard_id, ...fields }) => {
      ...
      const currentLinks = await db.select().from(schema.storyboardCharacters)
      ...
      await validateStoryboardBindings(
        ...
        'character_ids' in fields
          ...
          : currentLinks.map(link => link.characterId),
      )
